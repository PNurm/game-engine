...
public class TerrainMesh implements RenderableProvider {
    ...
    private final TerrainNode region;

    public TerrainMesh(TerrainNode region) {
        this.region = region;
        this.cell = region.getCell();
        ...
        material.set(new TerrainNode.Attr(TerrainNode.Attr.ATTRIBUTE_REGION, region));
    }
    ...
    public void setVertices() {
        for (int x = 0; x < terrainVertexCount; x++) {
            for (int z = 0; z < terrainVertexCount; z++) {
                ...
                TerrainNode.Tile tile;
                if (x < WorldCell.SIZE && z < WorldCell.SIZE) {
                    tile = region.getTile(x, z);
                } else
    ...
    public Vector3 position(Vector3 pos, float worldX, float worldY) {
        TerrainNode.Tile tt = WorldManager.worldTile(worldX, worldY);
    ...
    public void getRenderables(Array<Renderable> renderables, Pool<Renderable> pool) {
        ...
        r.worldTransform.set(region.getTransform());
        r.material = material;
        r.meshPart.offset = 0;
